...
use crate::*;
#[derive(Clone)]
pub struct PIDControllerStreamNode {
    pub in_node: Option<Rc<RefCell<Box<dyn CodeGenNode>>>>,
    pub var_name: Option<String>,
}
impl CodeGenNode for PIDControllerStreamNode {
        ...
        match target_version {
            TargetVersion::V0_3 => {
                let mut output = String::from(format!(
                    "let {} = make_input_getter!(PIDControllerStream::new(Rc::clone(&",
                    self.get_var_name()
                ));
                ...
                output.push_str("), setpoint, kvals), f32, E);\n");
                output
            }
            TargetVersion::V0_4 => {
                let mut output = String::from(format!(
                ...
                output.push_str(binding);
                output.push_str("), setpoint, kvals));\n");
                output
            }
        }
    }

...
use crate::*;
#[derive(Clone)]
pub struct CommandPIDNode {
    pub in_node: Option<Rc<RefCell<Box<dyn CodeGenNode>>>>,
    pub var_name: Option<String>,
}
impl CodeGenNode for CommandPIDNode {
    ...
    fn make_line(&self, target_version: TargetVersion) -> String {
        match target_version {
            TargetVersion::V0_3 => String::from(format!(
                "let {} = panic!(\"CommandPID available in RRTK 0.4\");\n",
                self.get_var_name()
            )),
            TargetVersion::V0_4 => {
                let mut output = String::from(format!(
                    "let {} = make_input_getter(CommandPID::new(Rc::clone(&",
                    self.get_var_name()
                ));
                ...
                output.push_str(", command, kvalues)));\n");
                output
            }
        }
    }
